...
test('bind-email skips 2925 cleanup errors and still submits add-email', async () => {
  const events = [];
  const logs = [];
  let runtimeState = {
    email: '',
  ...
  const executor = api.createStep8Executor({
    ...
    resolveSignupEmailForFlow: async () => 'bind.skip-cleanup@2925.com',
    reuseOrCreateTab: async () => 1,
    sendToContentScriptResilient: async (_source, message) => {
      if (message.type === 'GET_LOGIN_AUTH_STATE') {
        events.push(['auth-state']);
      ...
      return {
        submitted: true,
        displayedEmail: 'bind.skip-cleanup@2925.com',
        url: 'https://auth.openai.com/email-verification',
      };
    },
    sendToMailContentScriptResilient: async () => {
      events.push(['cleanup-error']);
      return { ok: false, error: 'empty inbox cleanup reported false' };
    },
    setState: async (payload) => {
      runtimeState = {
        ...runtimeState,
  ...
  assert.deepStrictEqual(events, [
    ...
    ['reopen2925'],
    ['cleanup-error'],
    ['submit-add-email', 'bind.skip-cleanup@2925.com'],
    ['persist', 'bind.skip-cleanup@2925.com'],
  ]);
  assert.equal(logs.some(({ message, level }) => (
    level === 'warn' && /清空 2925 收件箱失败，已跳过清空并继续提交绑定邮箱/.test(message)
  )), true);
});
...
test('step 8 uses a fixed 10-minute lookback window and plans 2925 polling as 10/10/10', async () => {
  ...
  assert.equal(capturedOptions.initialPollMaxAttempts, 10);
  assert.deepStrictEqual(capturedOptions.pollAttemptPlan, [10, 10, 10]);

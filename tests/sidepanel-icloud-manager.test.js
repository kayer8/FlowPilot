...
test('sidepanel does not render extension update card', () => {
  const html = fs.readFileSync('sidepanel/sidepanel.html', 'utf8');
  const css = fs.readFileSync('sidepanel/sidepanel.css', 'utf8');

  assert.doesNotMatch(html, /id="btn-ignore-release"/);
  assert.doesNotMatch(html, /id="btn-open-release"/);
  assert.doesNotMatch(html, /class="update-card/);
  assert.doesNotMatch(css, /\.update-card/);
  assert.doesNotMatch(css, /\.update-release/);
});

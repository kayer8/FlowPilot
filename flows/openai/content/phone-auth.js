(function attachPhoneAuthModule(root, factory) {
  ...
  function createPhoneAuthHelpers(deps = {}) {
    ...
    const PHONE_RESEND_BANNED_NUMBER_PATTERN = /无法向此电话号码发送(?:短信|文本消息)|无法向此手机号发送(?:短信|文本消息)|无法发送(?:短信|文本消息)到此电话号码|无法发送(?:短信|文本消息)到此手机号|この電話番号に(?:SMS|テキスト|メッセージ)を送信できません|電話番号に送信できません|can(?:not|'t)\s+send\s+(?:an?\s+)?(?:sms|text(?:\s+message)?)\s+to\s+(?:this|that)\s+(?:phone\s+)?number|unable\s+to\s+send\s+(?:an?\s+)?(?:sms|text(?:\s+message)?)\s+to\s+(?:this|that)\s+(?:phone\s+)?number/i;
    const PHONE_RESEND_SERVER_ERROR_PATTERN = /this\s+page\s+isn['’]?t\s+working|该网页无法正常运作|currently\s+unable\s+to\s+handle\s+this\s+request|http\s+error\s+500|500\s+internal\s+server\s+error/i;
